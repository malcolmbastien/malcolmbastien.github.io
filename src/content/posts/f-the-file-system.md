...
- Users need to be able to search only the appropriate types of files given what application they’re using.
- The system should support be full text search, letting users search entire document text, and they should be able to search on the meta-data. Such as “files created within the last week”.
- Present to the user the information they’ll most likely want. Do things like sort files by the order of when they were last modified, auto load the last open file into the application.
- Show them a single window to view all their data (think Simplenote, or Evernote) so they can skim their files.
- Hide the file system from them.
