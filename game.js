...
function drawStartScreen() {
    ctx.fillStyle = 'white';
    ctx.font = '30px Arial';
    ctx.fillText('Flappy Bird Clone', GAME_WIDTH / 2 - ctx.measureText('Flappy Bird Clone').width / 2, GAME_HEIGHT / 2 - 50);
    ctx.font = '20px Arial';
    ctx.fillText('Press Space to Start', GAME_WIDTH / 2 - ctx.measureText('Press Space to Start').width / 2, GAME_HEIGHT / 2);
}
...
function drawGameOverScreen() {
    ...
    ctx.fillStyle = 'white';
    ctx.font = '30px Arial';
    ctx.fillText('Game Over!', GAME_WIDTH / 2 - ctx.measureText('Game Over!').width / 2, GAME_HEIGHT / 2 - 30);
    ctx.font = '20px Arial';
    ctx.fillText(`Score: ${score}`, GAME_WIDTH / 2 - ctx.measureText(`Score: ${score}`).width / 2, GAME_HEIGHT / 2 + 10);
    ctx.fillText('Press Space to Restart', GAME_WIDTH / 2 - ctx.measureText('Press Space to Restart').width / 2, GAME_HEIGHT / 2 + 40);
}
...
function draw() {
    ...
    for (let i = 0; i < pipes.length; i++) {
        ...
        // Draw bottom pipe
        ctx.fillRect(p.x, p.topHeight + PIPE_GAP_HEIGHT, PIPE_WIDTH, p.bottomHeight);
    }
